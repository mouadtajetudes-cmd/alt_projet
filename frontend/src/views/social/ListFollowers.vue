  ...
  <div class="user-connections">
    <div class="tab-content">
      <div v-if="loading" class="loading">Chargement...</div>
        ...
        <div v-if="followers.length === 0" class="empty">Aucun follower</div>
        <div v-else class="list">
          <div v-for="user in followers" :key="user.id_utilisateur" class="user-card">
...
const loadFollowers = async () => {
  try {
    ...
    emit('update-count', followerCount.value)
  } catch (err) {
    console.error(err)
    followers.value = []
  }
...
const loadAll = async () => {
  loading.value = true
  await Promise.all([loadFollowers()])
  loading.value = false
}
